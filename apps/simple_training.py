...
from needle.progress import tqdm, ProgressBar
import sys
...
def epoch_general_cifar10(dataloader, model, loss_fn=nn.SoftmaxLoss(), opt=None):
    ...
    right_num = 0
    pbar = ProgressBar(total=len(dataloader), desc="Training" if opt else "Evaluating", unit="batch")
    for batch_idx, (x, y) in enumerate(dataloader):
        ...
        current_acc = right_num / ((batch_idx + 1) * len(y))
        current_loss = total_loss / ((batch_idx + 1) * len(y))
...
def train_cifar10(model, dataloader, n_epochs=1, optimizer=ndl.optim.Adam,
    ...
    epoch_pbar = tqdm(range(n_epochs), desc="Epochs", unit="epoch")
    for i in epoch_pbar:
        ...
        epoch_pbar.set_postfix(acc=acc, loss=loss, time=f"{end_time-start_time:.1f}s")
        
        print(f"\nEpoch {i}: acc: {acc:.4f}, loss: {loss:.4f}, time: {end_time-start_time:.1f}s")
    return acc, loss
...
def epoch_general_ptb(data, model, seq_len=40, loss_fn=nn.SoftmaxLoss(), opt=None,
    ...
    batch_indices = list(range(0, nbatch-1, seq_len))
    pbar = ProgressBar(total=len(batch_indices), desc="Training" if opt else "Evaluating", unit="batch")
    ...
    for idx, i in enumerate(batch_indices):
        ...
        # Update progress bar with current metrics
        current_acc = right_num / total_example if total_example > 0 else 0
        current_loss = total_loss / total_example if total_example > 0 else 0
        pbar.set_postfix(loss=current_loss, acc=current_acc)
        pbar.update(1)
